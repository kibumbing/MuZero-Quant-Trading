...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (1, 1, 10)  # Dimensions of the game observation, must be 3 (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
...
class Cosine:
    ...
    def render(self):
        # print("Current Rate: " + math.exp(self.total_reward))
    ...
    def get_reward(self, action):
        if action == 0:
            # if self.hold == 0:
            #     self.buy_amount = self.state[:, :, -2].item()
            #     self.hold = 1

            if self.state[:, :, -2].item() <= self.state[:, :, -1].item():
                return 1
            else:
                return 0
        else:
            # if self.hold == 1:
            #     self.hold = 0
            #     self.rate *= self.state[:, :, -2].item() / self.buy_amount
    ...
    #         if self.actions[i] == 0:
    #             plt.plot([self.xs[i - 1], self.xs[i]], [self.ys[i - 1], self.ys[i]], color='red', marker='o')
    #         else:
    #             plt.plot([self.xs[i - 1], self.xs[i]], [self.ys[i - 1], self.ys[i]], color='blue', marker='o')
    #     plt.savefig('cos_result.png')
